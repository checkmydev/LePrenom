...
Deno.serve(async (req) => {
  ...
  try {
    const { prenom, sexe, nom = "Gerard", famille = "gerard" } = await req.json();
    ...
    // Cache (par prénom ET famille : le nom de famille change le verdict de compatibilité)
    const { data: cached } = await sb.from("analyses")
    ...
    const prompt = `Tu es expert en prénoms francophones. Analyse le prénom "${prenom}" `
      + `(${sexe === "f" ? "fille" : "garçon"}) associé au nom de famille "${nom}".\n`
      ...
      + `possibles avec le prénom et/ou "${nom}", ou "aucun notable"), compat_gerard (verdict sur "${prenom} ${nom}": `
    ...
    await sb.from("analyses").upsert(row, { onConflict: "prenom,famille" });
    return json(row);
  } catch (e) {
